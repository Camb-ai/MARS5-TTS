...
class Mars5TTS(nn.Module, ModelHubMixin):
    ...
    def vocode(self, tokens: Tensor) -> Tensor:
        ...
        # Check if kernel size is greater than input size
        if features.size(-1) < self.vocos.kernel_size:
            logging.error("Kernel size is greater than input size. Returning empty tensor.")
            return torch.empty(0)

        try:
            wav_diffusion = self.vocos.decode(features, bandwidth_id=bandwidth_id)
        except RuntimeError as e:
            logging.error(f"Error during vocoding: {e}. Trying alternative method.")
            # Alternative method: use a different bandwidth_id
            bandwidth_id = torch.tensor([2], device=self.device)  # 12 kbps
            try:
                wav_diffusion = self.vocos.decode(features, bandwidth_id=bandwidth_id)
            except RuntimeError as e:
                logging.error(f"Alternative method failed: {e}. Returning empty tensor.")
                return torch.empty(0)
    ...
    def tts(self, text: str, ref_audio: Tensor, ref_transcript: Optional[str] = None, 
        ...
        # get text codes. 
        text_tokens =  self.texttok.encode("<|startoftext|>"+text.strip()+"
